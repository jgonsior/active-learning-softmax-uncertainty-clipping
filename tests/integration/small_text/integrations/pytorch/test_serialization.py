...
class SerializationTest(unittest.TestCase):
    def test_and_load_with_file_str(self):
        ...
        self.assertFalse(
            dataset._data[0][PytorchTextClassificationDataset.INDEX_TEXT].is_cuda
        )

        clf_factory = KimCNNFactory(
            "kimcnn", 6, {"embedding_matrix": torch.rand(len(dataset.vocab), 100)}
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            assert_array_equal(
                np.concatenate([ind_initial, ind]), active_learner.indices_labeled
            )
            ...
            for i in range(len(weights_before)):
                assert_array_equal(
                    weights_before[i].cpu().detach().numpy(),
