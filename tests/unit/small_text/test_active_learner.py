...
class _PoolBasedActiveLearnerTest(object):
    ...
    def _get_classifier_factory(self):
        return SklearnClassifierFactory(
    ...
    def _get_random_dataset(self, num_samples=100):
        if self.multi_label:
            return SklearnDataset(
                *random_matrix_data("dense", "sparse", self.NUM_SAMPLES)
            )
        else:
            return SklearnDataset(
                *random_matrix_data("dense", "dense", self.NUM_SAMPLES)
            )
    ...
    def test_initialize_data_with_indices_ignored(self):
        ...
        self._test_initialize_data(
            x_indices_random[:10],
            x_indices_ignored=x_indices_random[10:15],
            retrain=True,
        )
        self._test_initialize_data(
            x_indices_random[:10],
    ...
    def test_initialize_data_with_validation_set_given(self):
        x_indices_initial = np.random.choice(np.arange(100), size=10, replace=False)
        self._test_initialize_data(
            x_indices_initial, retrain=True, x_indices_validation="random"
        )
        self._test_initialize_data(
            x_indices_initial, retrain=False, x_indices_validation="random"
        )

    def _test_initialize_data(
        self,
        x_indices_initial,
        retrain=True,
        ...
        if x_indices_validation == "random":
            ...
            x_indices_validation = np.random.choice(
                x_indices_labeled_indices, size=1, replace=False
            )
    ...
    def test_query_without_prior_initialize_data(self):
        ...
        with self.assertRaises(LearnerNotInitializedException):
            active_learner = PoolBasedActiveLearner(
                clf_factory, query_strategy_mock, ds
            )
    ...
    def test_query(self, num_samples=5):
        ...
        query_strategy_mock.query.assert_called_with(
            ANY, ds, ANY, x_indices_initial, y_initial, n=num_samples
        )
    ...
    def test_query_with_custom_representation(self, num_samples=5):
        ...
        query_strategy_mock.query.assert_called_with(
            ANY, custom_x, ANY, x_indices_initial, y_initial, n=num_samples
        )
    ...
    def test_query_with_custom_representation_invalid(self, num_samples=5):
        ...
        custom_x = np.random.rand(
            98, 3
        )  # <-- representation's first dimension differs from x
    ...
    def test_query_with_kwargs(self):
        ...
        query_strategy_mock.query.assert_called_with(
            ANY, ds, ANY, x_indices_initial, y_initial, n=num_samples, **kwargs
        )
    ...
    def test_update(self):
        ...
        with patch.object(
            PoolBasedActiveLearner, "_retrain", return_value=None
        ) as retrain_mock:
            active_learner = PoolBasedActiveLearner(
                clf_factory, query_strategy, dataset
            )
            ...
            assert_array_equal(
                np.concatenate([indices_initial, indices_new]),
    ...
    def test_update_with_ignored_samples(self):
        ...
        with patch.object(
            ...
            if self.multi_label:
                ...
                y_new = np.array(
                    [LABEL_IGNORED, 1, LABEL_IGNORED, 0, LABEL_IGNORED]
                )  # ignore samples 0, 2, and 4
            active_learner.update(y_new)
            assert_array_equal(
                np.concatenate([indices_initial, [indices_new[1], indices_new[3]]]),
                active_learner.indices_labeled,
            )

            if self.multi_label:
                assert_csr_matrix_equal(
                    vstack([y_initial, y_new[[1, 3], :]]), active_learner.y
                )
            else:
                assert_array_equal(
                    np.concatenate([y_initial, y_new[[1, 3]]]), active_learner.y
                )
    ...
    def test_update_with_validation_set_given(self):
        ...
        with patch.object(
            ...
            indices_validation = np.random.choice(
                indices_initial, size=10, replace=False
            )

            active_learner.initialize_data(
                indices_initial, y_initial, indices_validation=indices_validation
            )
    ...
    def test_update_reuse_model(self):
        ...
        active_learner = PoolBasedActiveLearner(
            clf_factory_mock, query_strategy, dataset, reuse_model=True
        )
    ...
    def test_update_duplicate_indices_in_labeled_pool(self):
        ...
        clf_factory = SklearnClassifierFactory(
            ConfidenceEnhancedLinearSVC(), self.NUM_CLASSES
        )
    ...
    def test_remove_label_at_with_validation_set_given(self):
        self._test_remove_label_at(
            index_to_remove=-1, retrain=True, indices_validation="random"
        )

    def _test_remove_label_at(
        self, index_to_remove=-5, retrain=False, indices_validation=None
    ):
        ...
        with patch.object(
            ...
            if indices_validation == "random":
                indices_validation = np.random.choice(
                    indices_initial, size=3, replace=False
                )

            active_learner.remove_label_at(
                active_learner.indices_labeled[index_to_remove],
                retrain=retrain,
                x_indices_validation=indices_validation,
            )

            if isinstance(y_new, csr_matrix):
                assert_csr_matrix_equal(
                    dataset.y[active_learner.indices_labeled[-4:]],
                    active_learner.y[-4:],
                )
                assert_csr_matrix_equal(
                    dataset.y[active_learner.indices_labeled[:11]],
                    active_learner.y[:11],
                )
            else:
                assert_array_equal(
    ...
    def test_ignore_sample_at(self):
        ...
        self._test_ignore_sample_without_label_change(
            ds, indices_initial, y_initial, unlabeled_indices[0]
        )
    ...
    def test_ignore_sample_at_with_retrain_true(self):
        ...
        unlabeled_indices = self._get_unlabeled_indices(indices_initial)
        self._test_ignore_sample_without_label_change(
            ds, indices_initial, y_initial, unlabeled_indices[0], retrain=True
        )
    ...
    def test_ignore_sample_at_with_label_removed(self):
        ...
        indices_initial = np.random.choice(np.arange(100), size=10, replace=False)
        y_initial = self._get_y_initial()
        self._test_ignore_sample_with_label_change(
            ds, indices_initial, y_initial, indices_initial[0]
        )
    ...
    def test_ignore_sample_at_with_label_removed_and_retrain_true(self):
        ...
        y_initial = self._get_y_initial()
        self._test_ignore_sample_with_label_change(
            ds, x_indices_initial, y_initial, x_indices_initial[0], retrain=True
        )

    def test_ignore_sample_at_with_label_removed_and_retrain_true_and_validation_set_given(
        self,
    ):
        ds = self._get_random_dataset(num_samples=100)
        indices_initial = np.random.choice(np.arange(100), size=10, replace=False)
        ...
        self._test_ignore_sample_with_label_change(
            ...
            indices_initial[0],
            retrain=True,
            indices_validation=indices_initial[1:3],
        )
    ...
    def _test_ignore_sample_without_label_change(
        self,
        dataset,
        indices_initial,
        y_initial,
    ...
    def _test_ignore_sample_with_label_change(
        self,
        dataset,
        indices_initial,
        y_initial,
        ...
        with patch.object(
            ...
            self.assertEqual(
                indices_initial.shape[0], active_learner.indices_labeled.shape[0]
            )
            self.assertEqual(indices_initial.shape[0], active_learner.y.shape[0])
            assert_array_equal(np.array([]), active_learner.indices_ignored)

            active_learner.ignore_sample_at(
                index_to_ignore, retrain=retrain, indices_validation=indices_validation
            )

        self.assertEqual(
            indices_initial.shape[0] - 1, active_learner.indices_labeled.shape[0]
        )
...
class PoolBasedActiveLearnerSingleLabelTest(
    unittest.TestCase, _PoolBasedActiveLearnerTest
):
    def setUp(self):
...
class PoolBasedActiveLearnerMultiLabelTest(
    unittest.TestCase, _PoolBasedActiveLearnerTest
):
    def setUp(self):
