...
class ClassificationTypeTest(unittest.TestCase):
    def test_from_str(self):
        self.assertEqual(
            ClassificationType.SINGLE_LABEL, ClassificationType.from_str("single-label")
        )
        self.assertEqual(
            ClassificationType.MULTI_LABEL, ClassificationType.from_str("multi-label")
        )
...
class ConstraintTest(unittest.TestCase):
    ...
    def test_with_invalid_single_label_constraint(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Invalid configuration: This query strategy requires "
        ):
