...
class _ClassifierBaseFunctionalityTest(object):
    ...
    def test_predict_on_empty_data(self):
        train_set = random_sklearn_dataset(
            10,
...
class SklearnClassifierSingleLabelTest(
    unittest.TestCase, _ClassifierBaseFunctionalityTest
):
    def _get_clf(self):
    ...
    def test_fit_with_multi_label_data_on_single_label_classifier(self):
        train_set = SklearnDataset(
            *random_matrix_data("dense", "sparse", num_samples=10, num_labels=3)
        )
        clf = self._get_clf()

        expected_str = (
            "Given labeling is recognized as multi-label labeling but the classifier "
        )
        with self.assertRaisesRegex(ValueError, expected_str):
            clf.fit(train_set)


class SklearnClassifierMultiLabelTest(
    unittest.TestCase, _ClassifierBaseFunctionalityTest
):
    def _get_clf(self):
