...
try:
    ...
    class SimplePytorchClassifier(PytorchClassifier):
        ...
        def __init__(
            self, num_classes, multi_label=False, class_weight=None, device=None
        ):
...
class SimplePytorchClassifierTest(unittest.TestCase):
    ...
    @patch("torch.cuda.is_available")
    @patch("torch.cuda.current_device")
    def test_get_default_criterion_multiclass(
        self, mock_current_device, mock_is_available
    ):
        mock_current_device.return_value = "0"
        mock_is_available.return_value = True
    ...
    @patch("torch.cuda.is_available")
    @patch("torch.cuda.current_device")
    def test_get_default_criterion_multilabel(
        self, mock_current_device, mock_is_available
    ):
        mock_current_device.return_value = "0"
        mock_is_available.return_value = True
    ...
    @patch("torch.cuda.is_available")
    @patch("torch.cuda.current_device")
    def test_initialize_class_weights_balanced(
        self, mock_current_device, mock_is_available
    ):
        mock_current_device.return_value = "0"
        mock_is_available.return_value = True
    ...
    @patch("torch.cuda.is_available")
    @patch("torch.cuda.current_device")
    def test_initialize_class_weights_invalid_value(
        self, mock_current_device, mock_is_available
    ):
        mock_current_device.return_value = "0"
        mock_is_available.return_value = True
    ...
    @patch("torch.cuda.is_available")
    @patch("torch.cuda.current_device")
    def test_initialize_class_weights_multi_label_warning(
        self, mock_current_device, mock_is_available
    ):
        mock_current_device.return_value = "0"
        mock_is_available.return_value = True
        ...
        with self.assertWarnsRegex(
            UserWarning, "Setting class_weight to 'balanced' is intended"
        ):
            clf.initialize_class_weights(train_set)
