...
class ClassificationType(Enum):
    ...
    def from_str(classification_type_str):
        if classification_type_str == "single-label":
            ...
            raise ValueError(
                "Cannot convert string to classification type enum: "
        ...
        def query(
            ...
            **kwargs,
            ...
            if classification_type is not None:
                ...
                if (
                    classification_type_ == ClassificationType.SINGLE_LABEL
                    and isinstance(y, csr_matrix)
                ):
                    raise RuntimeError(
                        f"Invalid configuration: This query strategy requires "
                        f"classification_type={str(classification_type_.value)} "
                        f"but multi-label data was encountered"
                    )
                elif (
                    classification_type_ == ClassificationType.MULTI_LABEL
